...
export default function AdminAuditLogs() {
  ...
  const load = () => {
    const allLogs = getAll<AuditLog>('auditLogs');
  ...
  const filtered = logs.filter(l => {
    const matchesSearch = l.adminUser.toLowerCase().includes(search.toLowerCase()) || 
  ...
  const handleExport = () => {
    downloadCSV('audit_logs', 
      ['ID', 'Admin User', 'Action', 'Target', 'Details', 'Status', 'Timestamp'],
      filtered.map(l => [
        l.id, 
        l.adminUser, 
        l.action, 
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md shadow-xl border border-gray-200 dark:border-gray-700">
            ...
            <div className="p-5 space-y-3 text-sm">
              ...
              {selected.details && (
                <div className="mt-3">
                  <span className="text-gray-500 dark:text-gray-400 block mb-2">Details</span>
