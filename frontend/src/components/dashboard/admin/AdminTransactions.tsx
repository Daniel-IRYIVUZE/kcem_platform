...
import { Search, Eye, X, ArrowLeftRight, CheckCircle, AlertTriangle, RotateCcw } from 'lucide-react';
...
export default function AdminTransactions() {
  ...
  const handleExport = () => {
    downloadCSV('transactions',
      ['ID', 'Reference', 'Hotel', 'Recycler', 'Payment Method', 'Status', 'Description', 'Date'],
      filtered.map(t => [
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
              <tr>
                {['ID', 'Reference', 'Hotel', 'Recycler', 'Payment Method', 'Status', 'Date', ''].map(h => (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              {filtered.map(t => (
                <tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:bg-gray-900">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-600 dark:text-gray-400">{t.payment_method?.replace('_', ' ') || '—'}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <button onClick={() => setSelected(t)} className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400 transition-colors" title="View Details">
                        <Eye size={14} />
                      </button>
                      {t.status === 'pending' && (
                        <button onClick={() => handleStatusChange(t, 'completed')} className="p-1.5 rounded hover:bg-green-50 dark:hover:bg-green-900/20 text-green-600 dark:text-green-400 transition-colors" title="Mark Completed">
                          <CheckCircle size={14} />
                        </button>
                      )}
                      {(t.status === 'pending' || t.status === 'completed') && (
                        <button onClick={() => handleStatusChange(t, 'disputed')} className="p-1.5 rounded hover:bg-orange-50 dark:hover:bg-orange-900/20 text-orange-600 dark:text-orange-400 transition-colors" title="Mark Disputed">
                          <AlertTriangle size={14} />
                        </button>
                      )}
                      {(t.status === 'disputed' || t.status === 'completed') && (
                        <button onClick={() => handleStatusChange(t, 'refunded')} className="p-1.5 rounded hover:bg-purple-50 dark:hover:bg-purple-900/20 text-purple-600 dark:text-purple-400 transition-colors" title="Refund">
                          <RotateCcw size={14} />
                        </button>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md shadow-xl border border-gray-200 dark:border-gray-700">
            <div className="flex justify-between items-center p-5 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-lg font-bold text-gray-800 dark:text-gray-200">Transaction Details</h3>
            ...
            <div className="p-5 border-t border-gray-200 dark:border-gray-700">
              <div className="flex gap-2 flex-wrap">
                {selected.status !== 'completed' && (
                  <button onClick={() => handleStatusChange(selected, 'completed')} className="flex items-center gap-1.5 px-3 py-1.5 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700 transition-colors">
                    <CheckCircle size={14} /> Complete
                  </button>
                )}
                {selected.status !== 'disputed' && (
                  <button onClick={() => handleStatusChange(selected, 'disputed')} className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-600 text-white rounded-lg text-sm hover:bg-orange-700 transition-colors">
                    <AlertTriangle size={14} /> Dispute
                  </button>
                )}
                {selected.status !== 'refunded' && (
                  <button onClick={() => handleStatusChange(selected, 'refunded')} className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white rounded-lg text-sm hover:bg-purple-700 transition-colors">
                    <RotateCcw size={14} /> Refund
                  </button>
                )}
                <button onClick={() => setSelected(null)} className="flex-1 border border-gray-300 dark:border-gray-600 rounded-lg py-1.5 text-sm hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  Close
                </button>
              </div>
            </div>
