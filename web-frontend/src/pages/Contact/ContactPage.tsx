...
const ContactInfoSection = ({ contactInfo }: { contactInfo: any[] }) => (
  <motion.div
    ...
    {contactInfo.map((info, index) => (
      <div key={index} className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div className="space-y-3">
          {info.items.map((item: any, itemIndex: number) => (
...
const ContactFormSection = ({ formData, formStatus, inquiryTypes, onFormDataChange, onFileChange, onRemoveFile, onSubmit, fileInputRef, getResponseTime }: any) => (
  <motion.div
    initial={{ opacity: 0, x: 20 }}
    animate={{ opacity: 1, x: 0 }}
...
const ContactFAQSection = ({ filteredFaqs, openFaq, searchQuery, onSearchChange, onFaqToggle }: any) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
...
const ContactChatWidgetSection = ({ chatOpen, chatMessages, chatInput, onChatToggle, onChatInputChange, onChatSubmit }: any) => (
  <>
    <AnimatePresence>
      {chatOpen && (
        <motion.div
